...
**FABRIC_SQL_CONNECTION_URL_AGENTIC**: This is the connection string for the SQL Database that contains both **the agentic application's operational data** (e.g., chat history) and **the sample customer banking data**. You can find this in your Fabric workspace by navigating to the SQL database item called `agentic_app_db`, selecting first "..." then the "settings" -> "Connection strings" -> go to "ODBC" tab and select and copy SQL connection string.
